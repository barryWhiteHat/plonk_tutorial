...
def verify_naieve(Ql, Qr, Qm, Qo, Qc, a, b, c, witness_y, perm_a, perm_b, perm_c):
    ...
    x, Y , Px_b, rlc = copy_constraint_simple(range(len(a),len(a)*2), Xcoef, witness_y, v1, v2)
    x, Y , Px_c, rlc = copy_constraint_simple(range(len(a)*2,len(a)*3), Xcoef, witness_y, v1, v2)

    # calcualte permutated polynomial    
    x_1, Y_1 , Px_a_prime, rlc_1 = copy_constraint_simple(range(0,len(a)), perm_a, witness_y, v1, v2)
    x_1, Y_1 , Px_b_prime, rlc_1 = copy_constraint_simple(range(len(a),len(a)*2), perm_b, witness_y, v1, v2)
    x_1, Y_1 , Px_c_prime, rlc_1 = copy_constraint_simple(range(len(a)*2,len(a)*3), perm_c, witness_y, v1, v2)
